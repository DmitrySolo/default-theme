	...
	BX.Sale.OrderAjaxComponent = {
        ...
        modalAlertYN: function (msg) {
            ...
            this.modalAlert(msg.response.COMMENT,{
                ...
                callback:function(){
                    if(msg.data.bill1) {
                        obj.sendAjax1C(obj, msg.data.bill1, function () {
                            if(msg.data.bill2) obj.sendAjax1C(obj, msg.data.bill2);
                        }, msg.data.bill2?true:false);
                    }else obj.sendAjax1C(obj, msg.data.bill2, function(){});
                }
            },{
        ...
        sendAjax1C: function (obj, objSend, callback73, orderOne) {
            $.post( this.result.EXTRA.AJAX_PATH + "/ajax.php", objSend ,function( msg ) {
                if(msg.status == 'ok'){
                    if(msg.response.STRING == 'NOT_FOUND'){
                        obj.modalAlertYN(msg);
                        obj.bill_cut = msg.data.bill_cut;
                        return BX.PreventDefault(event);
                    }else{
                        if(obj.bill_cut) $('input[name="ORDER_PROP_75"]').attr('value',obj.bill_cut);
		...
		clickOrderSaveAction: function(event)
		{
			if (this.isValidForm())
			{
				...
				if(partner_id){
                    ...
                    this.sendAjax1C(this, objSend, function(){});
				}else{
		...
		{
			if(property.getId() >= 72 && property.getId() <= 75) return;
