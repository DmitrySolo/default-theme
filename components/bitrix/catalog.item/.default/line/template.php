...
<div class="productCatInline">
    <div class="productInline__content" data-qcontent="component__productInline">
        <div class="group group--md group--va-middle">
            <div class="col col-2-tp"><a class="productInline__link" href="<?= $item['DETAIL_PAGE_URL'] ?>">
                    <div title="<?= $imgTitle ?>"
                         data-entity="image-wrapper"
                         id="<?= $item['SECOND_PICT'] ? $itemIds['SECOND_PICT'] : $itemIds['PICT'] ?>"
                         class="img_container--align_v productInline__section--img"><div class="productCol__img" style="background-image: url(<?= $item['PREVIEW_PICTURE']['SRC'] ?>)"
                                                                                         title="<?= $imgTitle ?>" data-entity="image-wrapper"
                                                                                         id="<?= $item['SECOND_PICT'] ? $itemIds['SECOND_PICT'] : $itemIds['PICT'] ?>"></div></div>
                </a></div>
            <div class="col col-4-tp">
                ...
                <a class="productInline__link" href="<?= $item['DETAIL_PAGE_URL'] ?>"><span
                            class="productInline__section--sku">Код товара: <?= $product_sku ?></span>
                    <h6 class="productInline__section--name"><?= $item['NAME'] ?></h6></a>
            </div>
            <div class="col col-2-tp">
                <div class="productInline__section--price">
                    <?if($partnerId):?>
                        <?if($_COOKIE['shopState'] == 'true'):?>
                            <div class="priceAndStock priceAndStock--sale" data-qcontent="component__priceAndStock"><span
                                    ...
                                    class="productCol__stock productCol__stock--out">Нет в наличии</span><? endif ?><? if ($product_reserv): ?><span
                    ...
                    <?else:?>
                        <div class="priceAndStock priceAndStock--sale" data-qcontent="component__priceAndStock"><span
                                    class="productCol__price" id="<?= $itemIds['PRICE'] ?>"><? if (isset($arItemData['PRICES']['OLD_PRICE'])): ?><span
                ...
                <div class="col col-2-tp col-8-m push-2-m">
                    <div class="counter__ctn">
                        <button type="button" class="counter__btn counter__btn--minus"
                                id="<?= $arItemData['QUANTITY']['MINUS']['ID'] ?>">-
                        </button>
                        ...
                        <button class="counter__btn counter__btn--plus"
                                id="<?= $arItemData['QUANTITY']['PLUS']['ID'] ?>">+
                        </button>
                    </div>
                </div>
            <? endif ?>
            <div class="col col-2-tp"<? if (isset($arItemData['BUTTONS']['BUY'])) echo ' id="' . $arItemData['BUTTONS']['BUY']['ID_CONTAINER'] . '"' ?>>
                <? if (isset($arItemData['BUTTONS']['BUY'])): ?>
                    <button type="button" class="toOrder active notEdit"
                            data-qcontent="element__buttons__toOrder"
                            id="<?= $arItemData['BUTTONS']['BUY']['ID'] ?>"><?= $arItemData['BUTTONS']['BUY']['MESSAGE'] ?></button>
                <? endif ?>
            ...
            <div class="col col-12-tp"><!-- split modules/productChar -->
                    ...
                    <dl class="productChar">
                        ...
                        foreach ($item['DISPLAY_PROPERTIES'] as $code => $displayProperty) {
                            ...
                            <div class="productChar__terminDesc">
                                <dt><?= $displayProperty['NAME'] ?>:</dt>
                                <dd><?= (is_array($displayProperty['DISPLAY_VALUE'])
                                        ? implode(', ', $displayProperty['DISPLAY_VALUE'])
                                        : $displayProperty['DISPLAY_VALUE']) ?></dd>
                            </div>
                            <?
                        }
                        ?>
