...
$arResult["VIEW_STRING"]=sprintf('<div class="nonebx_catalog_view"><span>Вид: </span>
<a href="%s" class="'.$arParams["ELEMENT_VIEW_CUSTOM_TILES"].'">плитки</a> |
<a href="%s" class="'.$arParams["ELEMENT_VIEW_CUSTOM_LIST"].'">список</a></div>',
    $APPLICATION->GetCurPageParam("view=tiles",array("view"), false),
    $APPLICATION->GetCurPageParam("view=list",array("view"), false)
);
	...
	foreach ($arResult['ITEMS'] as $key => $arItem)
	{
		...
		if (!empty($arItem['DISPLAY_PROPERTIES']))
		{
			foreach ($arItem['DISPLAY_PROPERTIES'] as $propKey => $arDispProp)
			{
				...
				if($arDispProp['CODE']=="stock_reserved") {
                    $arItem['RESERV'] = intval($arDispProp['DISPLAY_VALUE']);
                    unset($arItem['DISPLAY_PROPERTIES'][$propKey]);
                }
			}
		}
